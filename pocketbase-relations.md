...
2. **递归删除场景的兜底保护**：`deleteRefRecords` 里触发级联时调用的是 `app.Delete(refRecord)`，会完整走一遍删除流程。根据 [runInTransaction](file:///d:/fz/0601/solo-dogfeeding/code/153-pocketbase/core/db_tx.go#L25-L49) 的实现——如果当前 db 已经是 `*dbx.Tx`，嵌套的 `RunInTransaction` **不会另开 savepoint，而是直接复用当前事务执行回调**——这意味着：
   ...
   - 同一事务内满足"读己之写"：前面递归 DELETE 掉的记录，对同一个事务后续的 SELECT 是可见的（后续查询就查不到已删记录了）
...
2. **同一事务内满足"读己之写"（Read-Your-Own-Writes）**：前面执行的 DELETE/UPDATE，对同一个事务后续的 SELECT 是可见的。比如引用记录 A 被递归删除后，后续循环里的查询就查不到 A 了。这里要注意区分两个概念：
   - **"读己之写"**：同一事务内，自己写的数据自己能读到——这是所有事务隔离级别都保证的基本特性，和隔离级别无关。
   - **"事务隔离"**：不同事务之间的可见性。SQLite 默认 SERIALIZABLE，事务 A 看不到事务 B 未提交的写入；同时其他并发事务在当前事务提交前也看不到当前事务的写入。
   
   之前容易混淆的点：PocketBase 级联删除复用同一个事务，所以递归之间看到彼此的写入是"读己之写"在起作用，不是"没有事务隔离"——事务隔离依然存在，只是隔离的是**外部其他事务**，而同一个事务内部的读写天然互相可见。
